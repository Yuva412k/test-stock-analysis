...
export const Holdings: React.FC = () => {
  const { data: holdings, isLoading: isHoldingsLoading } = useQuery<Holding[]>('holdings', fetchHoldings);
  const { data: sectors, isLoading: isSectorsLoading } = useQuery<string[]>('sectors', fetchSectors);
  ...
  if (isHoldingsLoading || isSectorsLoading) return <Loader />;
    ...
    <Card className="w-full bg-[#1b1a1a] text-white">
      ...
      <CardContent>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              {['Name', 'Quantity', 'Average Price', 'Current Price', 'Value', 'Profit/Loss', 'P/L %'].map((header, index) => (
                <TableHead
                  key={index}
                  onClick={() => handleSort(header.toLowerCase().replace(/\s/g, '') as keyof Holding)}
          ...
          <TableBody>
            {filteredAndSortedHoldings?.map(holding => (
              <TableRow key={holding.id}>
                ...
                <TableCell className={holding.profitLossPercentage >= 0 ? 'text-green-500' : 'text-red-500'}>
